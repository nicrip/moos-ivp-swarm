function values = readmseaspe_octave (strvar,lon,lat,depth,time);
%
% readmseaspe was written for Matlab by Patrick J. Haley, Jr. (MIT MSEAS Group)
%
% this script, readmseaspe_octave, has been modified for Octave from readmseaspe
%
% these modifications have been made by;
% Wayne G. Leslie (wgleslie@mit.edu, MSEAS) and Stephanie Petillo (spetillo@mit.edu, LAMSS)
%
...
%
% values = readmseaspe_octave (strvar,lon,lat,depth,time);
%
...
% ---------------
%
% The first call to readmseaspe_octave is always just to open the necessary PE
...
%
%    readmseaspe_octave({'Tpefile.nc' 'Vpefile.nc'});
%
...
%
%    readmseaspe_octave({{'smallest_pefile.nc' ... 'largest_pefile.nc'}});
%
% If the information comes from a set of L telescoping nested domain, each
...
%
%    readmseaspe_octave({{'smallest_Tpefile.nc' ... 'largest_Tpefile.nc'} ...
...
%
%    if ~isempty( readmseaspe_octave({{'smallest_Tpe.nc' ... 'largest_Tpe.nc'} ...
%                             {'smallest_Vpe.nc' ... 'largest_Vpe'}}, 1) )
%       error ('Unable to successfully open MSEAS PE file.');
%    end;
%
% If the file(s) were successfully opened, readmseaspe_octave returns the empty set
% ([]), otherwise it returns -1.
%
% Data Extraction:
% ----------------
%
% On subsequent calls, readmseaspe_octave interpolates the requested fields at
...
%
% For each requested field, readmseaspe_octave interpolates the PE netCDF file(s)
...
% size (N,6).  The output of readmseaspe_octave is a cell array of length M whose
...
% ---------
%
% When complete finished reading the file(s), call readmseaspe_octave with no
% arguments to close the files and %clear its internal storage.  If the file(s)
% were successfully closed, readmseaspe_octave returns the empty set ([]), otherwise it
% returns -1.
%
%    if ~isempty(readmseaspe_octave)
...
elseif ischar(strvar{1})
   ...
   disp ('Out of loop for two netcdf files');
...
for n = 1:ndom
   [domdatn,tlndindn,vlndindn,timdatn] = getpedom (nctid{n},tfile{n},ncvid{n},vfile{n});
   domdat(n,:) = domdatn;
   tlndind{n}  = tlndindn;
   vlndind{n}  = vlndindn;
   timdat(n,:) = timdatn;
...
if ~piflag
   ...
   %timeoff = datenum_alt(get_petim0(nctid)(1),get_petim0(nctid)(2),get_petim0(nctid)(3),get_petim0(nctid)(4),get_petim0(nctid)(5),get_petim0(nctid)(6));
   if isempty(timeoff)
      disp (' ');
   ...
   %timeoff = datenum_alt(gregorian(t0+jdoff)(1),gregorian(t0+jdoff)(2),gregorian(t0+jdoff)(3),gregorian(t0+jdoff)(4),gregorian(t0+jdoff)(5),gregorian(t0+jdoff)(6)) - t0; % a subtractive not additive factor.
   %clear jdoff
   timescale = 1;
end;
...
% due to changes in clos1mseas (making stw = [] always), this status check doesn't do anything anymore...
if ~isempty(status)
   status = -1;
end;
...
% since st1 = st2 = [] always, then status = [] always, so this status check doesn't do anything anymore...
if ~isempty([st1 st2])
   status = -1;
end;
...
%tt = (datenum_alt(time(1),time(2),time(3),time(4),time(5),time(6))' - timdat(1)).*timdat(3);
...
%tv = (datenum_alt(time(1),time(2),time(3),time(4),time(5),time(6))' - timdat(1)).*timdat(3);
