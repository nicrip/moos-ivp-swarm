  ...
  if (iproc)
    ...
    unwind_protect
      try
        ...
        do
          ## get command
          [err, msg] = ferror (cmdr(iproc));
          if err>0
            pid = getpid();
            system(["logger \"error :",num2str(pid),"\"",msg,"\n"]);
            break
          end
    ...
    unwind_protect
      ...
      while (pjobs < njobs || any (pending))
        ...
        if (isubp > 0)
          fprintf(stderr,"parcellfun: %d ready\n",pids(isubp));
          fflush(stderr);
          ijob = pending(isubp);
          ...
          if (pending(isubp))
            ...
            fprintf (stderr, "parcellfun: premature exit of process %d\n",pids(isubp));
            fflush (stderr);
            # fclose(cmdw(isubp));
            continue;
          endif
        ...
        if (pjobs < njobs)
          ...
          fflush (cmdw(isubp));
          if (verbose_level > 0)
            fprintf (stderr, "parcellfun: send terminate to %d\n",pids(isubp));
            fflush (stderr);
          endif
          terminated(isubp) = 1;
        endif
        if (verbose_level > 0)
          fprintf (stderr, "parcellfun: %d/%d jobs done\n", pjobs - sum (pending != 0), njobs);
          fflush (stderr);
        endif
      endwhile
    ...
    unwind_protect_cleanup
      ...
      for isubp = find (pending)
        ...
        fflush (cmdw(isubp));
        if (verbose_level > 0)
          fprintf (stderr, "parcellfun: exit of pending process %d\n",pids(isubp));
          fflush (stderr);
        endif
        terminated(isubp) = 1;
      endfor
      ## send termination signal to proceesses that did not get it already
      for isubp = find (terminated==0)
        fwrite (cmdw(isubp), 0, "double");
        fflush (cmdw(isubp));
        if (verbose_level > 1)
          fprintf (stderr, "parcellfun: exit of forgotten process %d\n",pids(isubp));
          fflush (stderr);
        endif
        terminated(isubp) = 1;
      endfor
      ...
      for i = 1:nproc
        ...
        fclose (cmdw(nproc)); # we can close the pipe now, because the process is finished
      endfor
