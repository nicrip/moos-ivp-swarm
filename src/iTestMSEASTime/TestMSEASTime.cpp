...
TestMSEASTime::TestMSEASTime() : m_reset_time_flag(false), m_model_pos_setflag(false), m_model_varname_setflag(false), m_appcasting_ready(false), m_start_flag(false), m_end_flag(false), num_req(0)
{
  m_iterations = 0;
  m_timewarp   = 1;
}
...
bool TestMSEASTime::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    } else if (msg.GetKey() == "START_TEST") {
      m_x_prev = m_x_pos;
      m_y_prev = m_y_pos;
      cout << "STARTING TEST!" << endl;
      m_start_flag = true;
    } else if (msg.GetKey() == "END_TEST") {
      m_end_flag = true;
...
bool TestMSEASTime::HandleNewNodeReport(CMOOSMsg msg)
{
  ...
  if (m_map_node_records.find(community_name) == m_map_node_records.end()) {
    ...
    m_x_pos = m_map_node_records[community_name].getX();
    m_y_pos = m_map_node_records[community_name].getY();
...
bool TestMSEASTime::OnConnectToServer()
{
...
bool TestMSEASTime::Iterate()
{
  ...
  double dist = sqrt(pow((m_x_pos-m_x_prev), 2) + pow((m_y_pos-m_y_prev), 2));
  cout << "distance: " << dist << endl;
  cout << "num req: " << num_req << endl;
  cout << "req time: " << m_request_time << endl;
  ...
  if (m_end_flag) {
    cout << "LAST REQ TIME: " << m_request_time << endl;
    myfile << endl;
    myfile.close();
  }
...
void TestMSEASTime::RequestMSEASData()
{
...
void TestMSEASTime::DistributeMSEASData()
{
...
bool TestMSEASTime::OnStartUp()
{
  ...
  m_end = clock();
  m_request_time = double(m_end-m_start)/CLOCKS_PER_SEC;
  cout << "time to open file " << m_model_filepath << ": " << m_request_time << " seconds." << endl;
  myfile << m_request_time << ", ";
...
void TestMSEASTime::RegisterVariables()
{
  ...
  Register("START_TEST", 0.0);
  Register("END_TEST", 0.0);
}

//---------------------------------------------------------
// Procedure: buildReport

bool TestMSEASTime::buildReport()
{
